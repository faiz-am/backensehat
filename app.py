...
def get_bigdata_foods():
    try:
        ...
        for r in rows:
            foods.append({
                "name": r[0],
                "calories": float(r[1]) if r[1] is not None else 0.0,
                "protein": float(r[2]) if r[2] is not None else 0.0,
                "fat": float(r[3]) if r[3] is not None else 0.0,
                "carbs": float(r[4]) if r[4] is not None else 0.0,
                "health_status": r[5]
            })
